...
const SubscribeMwForm = () => {
  const { address, accessToken }: any =
    useContext(GlobalContext)
  const [subscribeMw, setSubscribeMw] =
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      const edges =
        res?.data?.address?.wallet?.profiles
          ?.edges
      const profiles =
  ...
  const handleOnClick = (event: MouseEvent) => {
    ...
    if (
      target.className !== "dropdown-select-btn"
    ) {
  ...
  return (
    ...
    >
      ...
      <div className="form-post-middleware">
        ...
        <div>
          <label>
            <strong>FREE:</strong> users subscribe
            to your profile for free
            <input
              type="radio"
              name="middleware"
              value="free"
          ...
          <label>
            <strong>PAID:</strong> users pay 1
            LINK to subscribe to your profile
            <input
              type="radio"
              name="middleware"
              value="paid"
      ...
      <div className="form-note">
        <strong>Note:</strong> You will set the
        middleware for the selected profile.
      </div>
      <SetSubscribeBtn
