...
function CollectBtn({
  ...
  const [createCollectEssenceTypedData] =
    useMutation(CREATE_COLLECT_ESSENCE_TYPED_DATA)
  const [relay] = useMutation(RELAY)
  const [stateCollect, setStateCollect] =
    useState(isCollectedByMe)
  ...
  const handleOnClick = async () => {
    try {
      ...
      const typedDataResult =
        await createCollectEssenceTypedData({
          variables: {
            input: {
              options: {
                chainID: chainID,
