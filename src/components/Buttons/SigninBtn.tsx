import { useContext } from "react"
import { useMutation } from "@apollo/client"
import {
  LOGIN_GET_MESSAGE,
  LOGIN_VERIFY,
} from "../../utils/graphql"
import { DOMAIN } from "../../utils/helpers/constants"
import { GlobalContext } from "../../contexts/global"
import { Button } from "@chakra-ui/react"
import { useRouter } from "next/router"

function SigninBtn() {
  const router = useRouter()
  const {
    address,
    setAccessToken,
    connectWallet,
    checkNetwork,
  } = useContext(GlobalContext)
  // const { handleModal } = useContext(ModalContext);
  const [loginGetMessage] = useMutation(
    LOGIN_GET_MESSAGE
  )
  const [loginVerify] = useMutation(LOGIN_VERIFY)

  const handleOnClick = async () => {
    try {
      /* Connect wallet and get provider */
      const provider = await connectWallet()

      /* Check if the network is the correct one */
      await checkNetwork(provider)

      /* Get the signer from the provider */
      const signer = provider.getSigner()

      /* Get the address from the provider */
      const account = await (
        await signer
      ).getAddress()

      /* Get the network from the provider */
      const network = await provider.getNetwork()

      /* Get the chain id from the network */
      // const chainID = network.chainId;

      /* Get the message from the server */
      const messageResult = await loginGetMessage(
        {
          variables: {
            input: {
              address: account,
              domain: DOMAIN,
            },
          },
        }
      )
      const message =
        messageResult?.data?.loginGetMessage
          ?.message

      /* Get the signature for the message signed with the wallet */
      const signature = await (
        await signer
      ).signMessage(message)

      /* Verify the signature on the server and get the access token */
      const accessTokenResult = await loginVerify(
        {
          variables: {
            input: {
              address: account,
              domain: DOMAIN,
              signature: signature,
            },
          },
        }
      )
      const accessToken =
        accessTokenResult?.data?.loginVerify
          ?.accessToken
      console.log(
        "setting access token: ",
        accessToken
      )
      /* Save the access token in local storage */
      localStorage.setItem(
        "accessToken",
        accessToken
      )

      /* Set the access token in the state variable */
      setAccessToken(accessToken)

      /* Display success message */
      console.log(
        "success",
        "You are now logged in!"
      )

      router.reload()

      //   setTimeout(() => handleModal(null, ""), 1000);
    } catch (error: any) {
      /* Display error message */
      const message = error.message as string
      console.log("error", message)
    }
  }

  return (
    <>
      <Button
        bg={address ? "green.300" : "black"}
        _hover={{
          backgroundColor: address
            ? "green.300"
            : "black",
        }}
        color={address ? "white" : "gray.400"}
        fontSize={["md", "lg", "lg", "xl"]}
        w={["90%", "60%", "90%", "70%"]}
        mt="8px"
        disabled={address ? false : true}
        onClick={handleOnClick}
        cursor={"pointer"}
      >
        {!address ? "Connect Wallet " : "Sign In"}
      </Button>
    </>
  )
}

export default SigninBtn
